...
- [x] **`tauri-bridge.js::Store`/`WebviewWindow` retournaient des méthodes détachées** — le `Proxy` créé pour ces deux classes (`get(_, prop) { return Real[prop]; }`) renvoyait la propriété sans la lier (`bind`) à son objet d'origine ; si l'implémentation interne (minifiée, donc non auditable directement) utilise `this`, l'appeler via `Store.load(...)` aurait silencieusement perdu son contexte. Corrigé : `get()` lie maintenant toute propriété fonction à l'objet réel avant de la retourner.
- [x] **Listes de jeux/templates/engines vides ou panneaux qui ne s'affichent jamais, sans la moindre erreur console — bug CSS/JS systémique, pas un problème Tauri/Jocly** : `tabulon.css` définit `.hidden { display: none !important; }`. Plusieurs pages posent cette classe à l'état initial dans leur HTML sur des éléments que leur propre JS révèle ensuite *uniquement* via `el.style.display = ''`/`'none'` (jamais `classList.remove('hidden')`) — `!important` l'emporte toujours sur un style inline qui ne fixe aucune valeur explicite, donc ces éléments restaient invisibles pour toujours, sans qu'aucune exception ne soit levée nulle part (ni le HTML ni le JS n'ont de défaut de syntaxe, c'est juste deux mécanismes de visibilité incompatibles appliqués au même élément). Confirmé fichier par fichier : `hub.html` (4 panneaux : liste de jeux, templates, engines, about), `play.html` (`button-resume`, `button-favorite-yes`), `book.html` (liste des coups). Pas un bug dans `clock-setup.html` (10 occurrences) au même endroit, mais une vraie incohérence quand même : `clock-setup.js` gère très bien la visibilité par lui-même (`UpdateSymmetry()` masque tout puis révèle le bon sous-ensemble selon le mode horloge, dès le premier rendu) donc la classe `hidden` y était redondante, retirée par cohérence. **Vérifié et confirmé sans défaut** (toujours `classList.add/remove('hidden')`, jamais `style.display` sur ces mêmes éléments) : `engine.html`, `view-options.html`, `info.html` — rien à corriger ici. Pour les éléments corrigés sans logique d'affichage immédiate au chargement (`hub.html`, `play.html`, `book.html`), l'état masqué initial est conservé via `style="display:none"` inline, cohérent avec le mécanisme que leur JS utilise déjà — seule la classe CSS contradictoire est retirée.
...
- [x] **`hub.js` n'instanciait jamais `worker-bridge.js`** — `bridge.init()` n'était jamais appelé. `hub.js` importe maintenant `worker-bridge.js` et l'appelle. **Mais une tentative précédente de corriger en plus le chargement de Jocly dans le worker (voir ci-dessous) a cassé l'affichage de la liste de jeux** (`bridge.init()` bloquait alors tout le `DOMContentLoaded`, dont `ListGames()`, qui n'a jamais eu besoin du worker). Cette session revient donc à une approche plus prudente :
  - `ListGames()` et tout l'affichage de `hub.html` ne dépendent plus de `bridge.init()` : ce dernier est maintenant appelé **après**, sans `await`, avec son propre timeout (10s) et son propre log d'échec — un problème dans le worker n'empêche plus jamais la fenêtre hub de s'afficher et de naviguer.
  - **Diagnostic posé mais correction non retentée** : `match-worker.js::inject-jocly` fait `importScripts(args[0])` (URL de `jocly.js`) — or `importScripts()` n'existe pas dans un worker de type `module` (`new SharedWorker(url, {type:'module'})`, nécessaire puisque `match-worker.js` utilise `import`/`export` ES), confirmé par MDN : *"the method cannot be used in module workers"*. Cette ligne devrait donc échouer à chaque fois. Une correction (polyfill `importScripts` via XHR synchrone + indirect eval, chargement de `jocly.core.js` plutôt que `jocly.js`) avait été écrite puis abandonnée sans avoir pu être testée en conditions réelles — trop de mécanismes non vérifiables empiriquement depuis cette session (résolution de `self.exports`, comportement de `WorkerCreateGame()` une fois `Jocly.createMatch()` réellement appelée, etc.) pour l'appliquer à l'aveugle une deuxième fois. **Cette session se limite donc à instrumenter le code existant** (logs `[worker]`/`[bridge]`/`[hub]` à chaque étape, `try/catch` explicites, gestion de l'event `error` du `SharedWorker`) pour que le prochain test produise une trace exploitable plutôt qu'un nouveau silence à interpréter — voir la note ci-dessous sur où regarder.
  - **Limite connue de cette instrumentation** : les `console.log`/`error` ajoutés dans `match-worker.js` (préfixés `[worker]`) s'affichent dans la console **du SharedWorker lui-même**, pas dans celle de la fenêtre hub — il faut l'inspecter séparément (`chrome://inspect/#workers` ou équivalent selon le navigateur/WebView). Les logs `[bridge]` (dans `worker-bridge.js`, qui tourne côté fenêtre) et `[hub]` (dans `hub.js`), eux, apparaissent normalement dans la console de la fenêtre hub.
- [x] **Diagnostic complet du crash du `SharedWorker`, en deux temps** (session de debug interactive avec fhoudebert, beaucoup de tests `fetch()`/inspection DevTools en aller-retour) :
  1. **Faux problème, écarté** : un cache de build périmé (`src-tauri/target/`) faisait que tout fichier nouveau/modifié dans `app/` répondait `500` + `Content-Type: text/plain` au lieu du bon type — pas spécifique aux workers. Résolu en supprimant `target/` et en redémarrant `npm run dev`. **À refaire chaque fois qu'un nouveau fichier est ajouté sous `app/`**, semble-t-il : le dev server intégré de Tauri ne détecte pas toujours les nouveaux chemins après son démarrage initial.
  2. **Vrai problème, corrigé** : une fois ce premier problème écarté, `match-worker.js` continuait de planter à la construction du `SharedWorker` lui-même (avant tout échange de message). Confirmé par `fetch()` direct : `app/worker/*.js` est bien servi à la racine virtuelle sous `worker/...` (ex. `tauri://localhost/worker/match-worker.js`, 200, `text/javascript`) — **pas** sous `content/worker/...` (500). Or `worker-bridge.js::init()` construisait l'URL du worker avec `new URL('./match-worker.js', import.meta.url)`, et tout indique qu'`import.meta.url`, pour ce fichier précis (importé par `hub.js` via un chemin relatif remontant, `'../worker/worker-bridge.js'` depuis `app/content/hub.js`), ne se résolvait pas de façon fiable à son propre chemin réel. La cause exacte (bug du moteur de résolution de modules WebKit avec les imports relatifs remontants ? lien avec `frontendDist` en liste ?) n'a pas pu être isolée davantage dans cette session. Corrigé en construisant l'URL du worker avec un chemin absolu (`new URL('/worker/match-worker.js', location.origin)`), qui ne dépend d'aucune résolution de module et a été vérifié fonctionner par `fetch()` direct.
  3. **Vrai problème, plus profond, corrigé par changement d'architecture** : même avec le bon chemin, le `SharedWorker` continuait de planter à la construction, avec un event `error` générique sans aucun détail exploitable (`message`/`filename`/`lineno` tous `undefined` — limitation connue du spec pour les erreurs de parsing sur `SharedWorker`, voir whatwg/html#5323). Diagnostic posé par dichotomie systématique, en isolant chaque variable une à la fois (fichiers de test minimaux ajoutés/retirés au fil des échanges) :
     - Un `SharedWorker {type:'module'}` **sans aucun `import`** fonctionne.
     - Le même `SharedWorker {type:'module'}` **avec un seul `import` trivial** (`import {x} from './zzz.js'`, juste une constante) plante, peu importe ce qui est importé (testé avec `jb-engines.js` réel et un module minimal — même résultat).
     - Le même fichier, avec le même import, dans un **`Worker` dédié** `{type:'module'}` au lieu de `SharedWorker` : fonctionne parfaitement.
     - Conclusion : sur cette installation WebKitGTK (2.52.3), un `SharedWorker` module se construit et exécute son script racine correctement, mais la résolution des **imports internes** à ce script échoue silencieusement (à l'inverse d'un `Worker` dédié module, où le même mécanisme fonctionne). C'est un comportement de la plateforme, pas un bug du code de Tabulon — voir aussi le support plus tardif et moins uniforme des "SharedWorker modules" par rapport aux "dedicated worker modules" dans tous les moteurs (Chromium a séparé les deux Intent-to-Ship, Firefox ne les supporte toujours pas du tout pour SharedWorker).
     - **Correction** : `match-worker.js` est redevenu un script de worker **classique** (pas `type:'module'`) — viable ici car aucune fenêtre autre que `hub.js` ne se connecte directement à ce `SharedWorker` (toute communication avec les autres fenêtres passe par les events Tauri relayés par `worker-bridge.js`/Rust), donc rien n'exploitait la sémantique de partage propre à `SharedWorker` au-delà de cette unique connexion ; passer à un `Worker` dédié aurait été une option tout aussi valable. Changements : `import { createEngine } from './jb-engines.js'` → `importScripts('./jb-engines.js')` (et `export function createEngine` → simple `function createEngine`, désormais globale) ; `worker-bridge.js` retire `type: 'module'` du constructeur. Au passage, `inject-jocly` charge maintenant `jocly.core.js` (pas `jocly.js`, qui suppose `document`, absent de tout Worker — voir tentative précédente abandonnée) via `importScripts` natif (qui n'existait pas en worker module, d'où l'abandon précédent ; existe nativement en worker classique).
     - **Non vérifié dans cette session, à surveiller au prochain test** : `jocly.core.js::WorkerCreateGame` (utilisée par `Jocly.createMatch`, pas par `inject-jocly`) appelle elle-même `importScripts("jocly-allgames.js")` avec un chemin relatif court. Confirmé MDN : *"If the URL is relative, it is relative to the worker entry script's URL"* — donc ce chemin se résoudrait par rapport à `match-worker.js` (`worker/jocly-allgames.js`, qui n'existe pas), pas par rapport à `jocly.core.js` (`browser/jocly-allgames.js`, le bon emplacement), même si `jocly.core.js` est chargé en cascade depuis `match-worker.js`. À corriger si `new_match` échoue avec une erreur réseau sur ce fichier.
  4. **Limite d'observabilité levée** : le `SharedWorker` ne plante plus à la construction, mais `bridge.init()` continue de bloquer (timeout) sans qu'on puisse voir pourquoi — les `console.log` dans `match-worker.js` n'apparaissent que dans la console **du worker lui-même**, pas accessible facilement selon le navigateur/WebView (pas d'inspecteur de Worker trouvé sous Epiphany/WebKitGTK dans cette session). Ajout d'une fonction `workerLog()` dans `match-worker.js` qui logue localement *et* relaie le message au hub via `port.postMessage({type:'event', event:'worker-log', ...})`, que `worker-bridge.js::handleWorkerEvent` réaffiche dans la console de la fenêtre hub (préfixé `[from worker]`). Tous les `console.*` du handler `onconnect` (inject-jocly, sync-store, commandes) sont passés à `workerLog`.
