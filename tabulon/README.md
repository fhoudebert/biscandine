...
- [ ] **Icons** — déjà générées (set par défaut `cargo tauri icon`), à personnaliser si besoin.
...
Tabulon ne dépend pas de Jocly via npm : la bibliothèque est construite séparément
depuis [jocly2](https://github.com/fhoudebert/jocly2) (`gulp build`) puis son
dossier `dist/` est copié tel quel à la racine de `tabulon/` (pas dans
`node_modules/`) :

```bash
tabulon/
├── app/
├── dist/              ← copié depuis jocly2/dist/ après `gulp build` dans jocly2
│   └── browser/jocly.js, jocly.core.js, jquery.js, games/, ...
└── src-tauri/
```

```bash
# 0. Construire Jocly séparément, puis copier son dist/ à la racine de tabulon/
#    (voir jocly2/README.md pour le détail du build)
cp -r /chemin/vers/jocly2/dist /chemin/vers/tabulon/dist

# 1. Installer les dépendances racine
npm install

# 2. Installer les dépendances frontend (jquery, js-yaml, photonkit, @tauri-apps/*)
npm --prefix app install

# 3. Compiler le parser PJN — nécessite pjn-parser/*.jison, actuellement absent
#    du dépôt (voir Travaux restants) ; sans lui book-history.html ne fonctionnera pas
